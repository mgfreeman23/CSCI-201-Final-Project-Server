    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException{
        ...
        try {
			if(!rs.next()){
			    ...
			    ps = conn.prepareStatement("SELECT password, user_id FROM app_database.users WHERE username=?");
			    ps.setString(1, uname);
			    rs = ps.executeQuery();

			    rs.next();
			    if(!rs.getString(1).equals(pass)){ // wrong password: code -1
			        pw.write(gson.toJson(-1));
			        pw.flush();
			    }
			    else{ // if it does equal, return the userID for future use within user's session
			        pw.write(gson.toJson(rs.getInt(2)));
			        pw.flush();
			    }
			}
		} catch (SQLException e) {
